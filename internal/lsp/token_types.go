...
func TokenTypesLegend(clientSupported []string) TokenTypes {
	legend := serverTokenTypes

	// Remove unsupported token types
	for i, tokenType := range legend {
		if !sliceContains(clientSupported, string(tokenType)) {
			legend = append(legend[:i], legend[i+1:]...)
		}
	}
...
func TokenModifiersLegend(clientSupported []string) TokenModifiers {
	legend := serverTokenModifiers

	// Remove unsupported modifiers
	for i, modifier := range legend {
